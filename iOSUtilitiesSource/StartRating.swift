...
/**
 ...
 Functions for making StarRating view accessible.
 
 */
struct StarRatingAccessibility {
    /**
    ...
    static func update(_ view: UIView, rating: Double, text: String?, settings: StarRatingSettings) {
        view.isAccessibilityElement = true
        
        view.accessibilityTraits = settings.updateOnTouch ?
            UIAccessibilityTraits.adjustable :UIAccessibilityTraits.none
        
        var accessibilityLabel = StarRatingLocalizedRating.ratingTranslation
     ...
     */
    static func accessibilityValue(_ view: UIView, rating: Double, settings: StarRatingSettings) -> String {
        let accessibilityRating = StarRatingRating.displayedRatingFromPreciseRating(rating,
     ...
     */
    static func accessibilityIncrement(_ rating: Double, settings: StarRatingSettings) -> Double {
    ...
    static func accessibilityDecrement(_ rating: Double, settings: StarRatingSettings) -> Double {
        var increment: Double = 0
...
struct StarRatingDefaultSettings {
    ...
    /// When `true` the star fill level is updated when user touches the StarRating view. When `false` the StarRating view only shows the rating and does not act as the input control.
...
/**
 ...
 Helper class for calculating size for the StarRating view.
 
 */
class StarRatingSize {
    /**
     
     Calculates the size of the StarRating view. It goes through all the star and text layers and makes size the view size is large enough to show all of them.
...
class StarRatingLayers {
     ...
     */
    class func createStarLayers(_ rating: Double, settings: StarRatingSettings, isRightToLeft: Bool) -> [CALayer] {
        
        var ratingRemander = StarRatingRating.numberOfFilledStars(rating,
        ...
        for _ in (0..<settings.totalStars) {
            ...
            let fillLevel = StarRatingRating.starFillLevel(ratingRemainder: ratingRemander,
    ...
    /**
     ...
     - parameter settings: Star view settings.
     - returns: Layer that shows the star. The layer is displayed in the StarRating view.
     
     */
    class func createCompositeStarLayer(_ starFillLevel: Double,
                                        settings: StarRatingSettings, isRightToLeft: Bool) -> CALayer {
     ...
     */
    class func createPartialStar(_ starFillLevel: Double, settings: StarRatingSettings, isRightToLeft: Bool) -> CALayer {
    ...
    private class func createStarLayer(_ isFilled: Bool, settings: StarRatingSettings) -> CALayer {
...
// ----------------------------
//
// StarRatingLocalizedRating.swift
//
// ----------------------------
 ...
 */
struct StarRatingLocalizedRating {
...
struct StarRatingTouch {
     ...
     */
    static func touchRating(_ position: CGFloat, settings: StarRatingSettings) -> Double {
        ...
        rating = StarRatingRating.displayedRatingFromPreciseRating(rating,
                                                               fillMode: settings.fillMode, totalStars: settings.totalStars)
...
public struct StarRatingSettings {
    ...
    /// Returns default set of settings for StarRatingView
    public static var `default`: StarRatingSettings {
    ...
    /// Border color of an empty star.
    public var emptyBorderColor = StarRatingDefaultSettings.emptyBorderColor
    
    /// Width of the border for empty star.
    public var emptyBorderWidth: Double = StarRatingDefaultSettings.emptyBorderWidth
    
    /// Border color of a filled star.
    public var filledBorderColor = StarRatingDefaultSettings.filledBorderColor
    
    /// Width of the border for a filled star.
    public var filledBorderWidth: Double = StarRatingDefaultSettings.filledBorderWidth
    
    /// Background color of an empty star.
    public var emptyColor = StarRatingDefaultSettings.emptyColor
    
    /// Background color of a filled star.
    public var filledColor = StarRatingDefaultSettings.filledColor
    ...
    /// Set to `false` if you don't want to pass touches to superview (can be useful in a table view).
    public var passTouchesToSuperview = StarRatingDefaultSettings.passTouchesToSuperview
...
@IBDesignable open class StarRatingView: UIView {
     ...
     */
    @IBInspectable open var rating: Double = StarRatingDefaultSettings.rating {
        didSet {
            if oldValue != rating {
                update()
            }
    ...
    open var settings: StarRatingSettings = .default {
        didSet {
            update()
        }
    }
    ...
    /**
     ...
     Initializes and returns a newly allocated StarRating view object.
     
     */
    public convenience init(settings: StarRatingSettings = .default) {
        self.init(frame: .zero, settings: settings)
    }
    
    /**
     
     Initializes and returns a newly allocated StarRating view object with the specified frame rectangle.
    ...
    public init(frame: CGRect, settings: StarRatingSettings) {
    ...
    open func update() {
        ...
        var layers = StarRatingLayers.createStarLayers(
            rating,
            settings: settings,
    ...
    private func createTextLayer(_ text: String, layers: [CALayer]) -> CALayer {
        ...
        if RightToLeft.isRightToLeft(self) {
            StarRatingText.position(textLayer, starsSize: CGSize(width: 0, height: starsSize.height), textMargin: 0)
        } else {
    ...
    /**
     ...
     Prepares the StarRating view for reuse in a table view cell.
     If the StarRating view is used in a table view cell, call this method after the
    ...
    private func updateAccessibility() {
        StarRatingAccessibility.update(self, rating: rating, text: text, settings: settings)
    }
    ...
    open override func accessibilityIncrement() {
        ...
        rating += StarRatingAccessibility.accessibilityIncrement(rating, settings: settings)
        didTouchStarRating?(rating)
        didFinishTouchingStarRating?(rating)
    }
    ...
    open override func accessibilityDecrement() {
        ...
        rating -= StarRatingAccessibility.accessibilityDecrement(rating, settings: settings)
        didTouchStarRating?(rating)
        didFinishTouchingStarRating?(rating)
    }
    ...
    /// Closure will be called when user touches the StarRating view. The touch rating argument is passed to the closure.
    open var didTouchStarRating: ((Double)->())?
    
    /// Closure will be called when the user lifts finger from the StarRating view. The touch rating argument is passed to the closure.
    open var didFinishTouchingStarRating: ((Double)->())?
    ...
    func onDidTouch(_ locationX: CGFloat) {
        let calculatedTouchRating = StarRatingTouch.touchRating(locationX, settings: settings)
        ...
        if calculatedTouchRating == previousRatingForDidTouchCallback {
            // Do not call didTouchStarRating if rating has not changed
            return
        }
        
        didTouchStarRating?(calculatedTouchRating)
    ...
    override open func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let oprimizedBounds = StarRatingTouchTarget.optimize(bounds)
    ...
    @IBInspectable var filledColor: UIColor = StarRatingDefaultSettings.filledColor {
        didSet {
            settings.filledColor = filledColor
        }
    }
    
    @IBInspectable var emptyColor: UIColor = StarRatingDefaultSettings.emptyColor {
        didSet {
            settings.emptyColor = emptyColor
        }
    }
    
    @IBInspectable var emptyBorderColor: UIColor = StarRatingDefaultSettings.emptyBorderColor {
        didSet {
            settings.emptyBorderColor = emptyBorderColor
        }
    }
    
    @IBInspectable var emptyBorderWidth: Double = StarRatingDefaultSettings.emptyBorderWidth {
        didSet {
            settings.emptyBorderWidth = emptyBorderWidth
        }
    }
    
    @IBInspectable var filledBorderColor: UIColor = StarRatingDefaultSettings.filledBorderColor {
        didSet {
            settings.filledBorderColor = filledBorderColor
        }
    }
    
    @IBInspectable var filledBorderWidth: Double = StarRatingDefaultSettings.filledBorderWidth {
        didSet {
